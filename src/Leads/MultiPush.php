 ...
 * Class MultiPush
 * @package Leads\Leads
 * Для использования метода multiPush необходимо подключить специальный оффер «Мультилид»
 *
 ...
 *
 * @property string algorithm array JSON
 * Пример:
 * [
 *   {"offers":[26,343,234]},
 *   {"offers":[12,332,545],"pause":360},
 *   {"offers":[4334,442,22],"pause":180}
 * ]
 * для первой группы: 26,343,234 анкета будет отправлена сразу же, без паузы.
 * Если хотя бы одна анкета была успешной (рекламодатель принял ее по API без отказов),
 * то через 360 минут будет запущена отправка анкет на вторую группу офферов 12,332,545.
 * Если же все анкеты первой группы были отклонены при отправке рекламодателю, то
 * отправка на вторую группу офферов будет без паузы в 360 минут. Далее по этому же
 * принципу через 180 минут(или без паузы) анкета будет отправлена на третью группу. А
 * далее через 60 минут на остальные оффера, которые не были включены ни в одну из групп.
 *
 */
class MultiPush extends abstractObject
{
    public function __construct($token)
    {
        $this->properties = [
            ...
            'own_funds', 'additional_info', 'test_mode', 'algorithm',
        ];
        parent::__construct($token, 'leads/multiPush');
    }
